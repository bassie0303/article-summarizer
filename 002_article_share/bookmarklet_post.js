(function(){
  ...
  if(!sel){
    if(navigator.clipboard&&navigator.clipboard.readText){
      navigator.clipboard.readText().then(function(txt){if(txt&&!qEl.value){qEl.value=txt.trim();upd();}}).catch(function(){setTimeout(function(){qEl.focus();},50);});
    }else{setTimeout(function(){qEl.focus();},50);}
  }
  ...
  if(_savedGemKey){document.getElementById(Q+'gk').value=_savedGemKey;document.getElementById(Q+'gv').checked=true;}
  ...
  async function aiSuggest(prompt,targetId,btn){
    var k=document.getElementById(Q+'gk').value.trim();
    if(!k){alert('Gemini APIキーを入力してください（🔑 Gemini APIキー欄）');document.getElementById(Q+'gk').focus();return;}
    document.getElementById(Q+'gv').checked?localStorage.setItem('_gemk',k):localStorage.removeItem('_gemk');
